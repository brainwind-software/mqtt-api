...
export class Client {
  ...
  request(
    ...
    clientCallback?: PacketCallback,
  ): Promise<ResponseParameters>;
  request(
    topic: string,
    params: RequestParameters,
    options: RequestOptions,
    clientOpts: IClientPublishOptions,
    clientCallback?: PacketCallback,
  ): Promise<ResponseParameters>;
  request(
    topic: string,
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      let abortTimeout;
      const timeoutMs = options?.timeout !== undefined ? options.timeout : this.options.response.defaultTimeout;
      if (timeoutMs > 0) {
